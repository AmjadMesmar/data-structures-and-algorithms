...
const isCapitalized = (str) => {
  ...
  let regexVal = /[A-Z](\w)*/g;
  let regexMatch = str.match(regexVal);
  let arr = [];

  if (regexVal.test(str)){
    return str.match(regexVal);
  }
  return arr;
};
...
const citiesAtoJ = (arr) => {
  ...
  let regexVal = /^([A-J])\w*/g;
  // let check = arr.match(regexVal);
  let newArr = [];

  arr.forEach(val => {
    if (val.match(regexVal))
      newArr.push(val);
