...
const count = (target, input) => {
  // console.log('array of arrays',input);
  let sumCount =0;
  let regexVal = new RegExp(`\\b${target}\\b`, 'i');
  // let regexVal = /5/i;
  console.log('my regex',regexVal);
  let newArr = input.join(',').split(',');
  // console.log('joined array',newArr);
  newArr.forEach (val => {
    let check = regexVal.test(val);
    if (check)
      sumCount ++;
  });
  console.log('please run',sumCount);
  return sumCount;
};

// count (8,[[1,2,3,4,5],[5,6,7,5,5],[5,5,4,5]]);
...
let findShortest = (data) => {
  ...
  let starArray = filterValue.map(element=>
  {
    return element.name;
  });
  let deatStarArray = starArray.join( ' and ' );
  return deatStarArray;
};
