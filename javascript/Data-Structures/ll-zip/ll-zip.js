...
function zipLists(ls1,ls2){
  if(!ls1.head || !ls2.head){
    ...
    throw 'Invalid input, you must use linked lists!';
  }
  let newLL = new LinkedList;
  let ll1 = ls1.head;
  let ll2 = ls2.head;
  let lenght1 = ls1.calcLength();
  let lenght2 = ls2.calcLength();
  let newLength = lenght1+lenght2;

  for(let i=0; i<= newLength; i++){

    if(ll1){
      newLL.append(ll1.value);
      ll1 = ll1.next;
    }
    if(ll2){
      newLL.append(ll2.value);
      ll2 = ll2.next;
    }

  }

  console.log('Zipped Linked List: ',newLL.toString());
  return newLL;
